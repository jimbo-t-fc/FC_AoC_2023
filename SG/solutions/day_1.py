...
class CalibrationUnscrambler:
    ...
    def unscramble_calibration_file(
        self, output_file: str, convert_number_strings: bool = False
    ):
        output_lines = []
...
def main(input_dir="../input", output_dir="../output"):
    ...
    sol = unscrambler.unscramble_calibration_file(
        Path(output_dir) / "day_1_output_p1.txt"
    )
    print(sol)
    sol2 = unscrambler.unscramble_calibration_file(
        Path(output_dir) / "day_1_output_p2.txt", convert_number_strings=True
    )
    print(sol2)
